...
void initial_mine(int mine[X][Y], int x1, int y1)  //初始化雷的分布
{
	...
	while (n < M)
	{
		...
		x = 0 + rand() % X;  //生成随机坐标
		y = 0 + rand() % Y;
		if (x == x1 && y == y1)  //避免第一个坐标有雷
			;
		else if (mine[x][y] != -1)  //坐标上没有雷，且不是第一次
		{
			mine[x][y] = -1;
			for (i = x - 1; i <= x + 1; i++)
			{
...
int detect(int mine[X][Y], char board[X][Y], int x, int y)  //检测被选择的坐标及其周围有没有雷
{
	...
	case 0:  //坐标周围没有雷
		...
		//for (int i = 0; i < X; i++)  //检测坐标周围8格是否也没有雷
		//{
		//	for (int j = 0; j < Y; j++)
		//	{
		//		if (board[i][j] == ' ')
...
//递归条件：1.该坐标及其周围8个坐标不是雷；2.该坐标未被排查
void print0_better(int mine[X][Y], char board[X][Y], int x, int y)  //当坐标周围没有雷时用特殊的打印，改良版本
{
	int i = 0, j = 0, k = 0;
	for (i = -1; i <= 1; i++)
	{
		for (j = -1; j <= 1; j++)  //遍历该坐标周围的8个坐标
		{
			if ((mine[x + i][y + j] != -1) && (inboard(x + i, y + j)))  //该坐标不是雷
			{
				if (board[x + i][y + j] == 'X')  //该坐标未被排查
				{
					board[x + i][y + j] = '0' + mine[x + i][y + j];
					if (mine[x + i][y + j] == 0)  //该坐标及其周围的8个坐标都不是雷
					{
						board[x + i][y + j] = ' ';
