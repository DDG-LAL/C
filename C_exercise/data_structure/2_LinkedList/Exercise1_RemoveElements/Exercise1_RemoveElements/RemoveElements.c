...
struct ListNode* removeElements(struct ListNode* head, int val)
{
	...
	while (cur)
	{
		...
		{
			if (!newhead) //若新头指针为空，由第一个非val节点确定新头指针
				tail = newhead = cur; //尾指针和头指针都指向第一个节点
			else //头指针不为空，直接尾插
			{
				tail->next = cur; //将新链表尾节点链接到当前节点
				tail = tail->next; //tail重新指向新链表尾节点
			}
