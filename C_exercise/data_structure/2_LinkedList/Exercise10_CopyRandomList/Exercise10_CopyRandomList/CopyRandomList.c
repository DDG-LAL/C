...
struct Node* BuyNode(int x) //创建新节点
{
...
struct Node* copyRandomList(struct Node* head) //在原链表的每个节点后插入一个复制节点
{											   //利用复制节点与原链表节点的位置关系确定复制节点的random
	if (!head)								   //确定random的值后分离出新节点和旧节点，形成新旧两个相同链表，返回新链表
		return NULL;
	struct Node* cur = head, * next = head->next;
	while (cur) //在原链表的每个节点后插入一个复制节点
	{
		struct Node* newnode = BuyNode(cur->val); //复制节点的val值与原链表相同
		newnode->next = next;
		cur->next = newnode;
		cur = next;
		if (next) //避免越界
	...
	cur = head;
	next = head->next;
	while (cur) //利用复制节点与原链表节点的位置关系确定复制节点的random
	{
		next->random = cur->random ? cur->random->next : NULL; //若原节点random值不为NULL，则新节点random值为原节点的random指向的节点的next
		cur = cur->next->next;
		if (next->next) //避免越界
		{
	...
	struct Node* newhead = head->next; //保存新链表头节点
	...
	{
		pre->next = next;
		cur->next = next ? next->next : NULL; //若next不为NULL，则新节点的next指针指向next->next
		pre = next;
		if (next) //避免越界
		{
			cur = next->next;
